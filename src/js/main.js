...
const changeQuantityRoom =
  document.getElementsByClassName("changeQuantityRoom");
...
const update = () => {
  ...
  for (let i = 0; i < changeQuantityRoom.length; i++) {
    ...
    valueTotalPrice +=
      parseInt(priceRoom[i].textContent) *
        parseInt(changeQuantityRoom[i].value) || 0;
  }
...
const commentBlock = () => {
  const display = document.getElementsByClassName(
    "detail_block_seven_comment_hiden"
  );
